...
async def info(event):
  await event.reply("**Merhaba Ben İnek Tagger Bot\n\nGrubunuzda Veya Kanalınızda Sizin İçin Üyeleri Etiketlerim\nOyun & Film Botumuz: @inekgame_bot\n\nBeni Bir Gruba Ekleyerek Başlayabilirsin**",
                    buttons=(
                      [
...
async def start(event):
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     return await event.reply(f"**Merhaba \nBen Grubunuzdaki Üyeleri Etiketlerim\nKomutlar için Komutlar Butonuna Tıklayın**", buttons=(
                      [
  ...
  if event.is_group:
    return await client.send_message(event.chat_id, f"**Etiketlemeye Başlamam İçin Komutlar Menüsünden Komut Verin 🔱**")
...
async def mentionall(event):
  ...
  if event.is_private:
    return await event.respond("**Bu Komut Sadace Gruplarda veya Kanallarda Kullanılabilir**")
  ...
  if event.pattern_match.group(1):
    ...
    if msg == None:
        return await event.respond("**Eski Mesajlar İle Üyelerden Bahsedemem! (gruba eklemeden önce gönderilen mesajlar)**")
...
async def mentionalladmin(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Bu Komut Yalnızca Grublarda Ve Kanallarda Kullanılabilir!**")
  ...
  if mode == "text_on_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    await event.respond("**Admin Etiket İşlemi Başlatıldı.!**")
